...
class Wind(gtk.Window):
    ...
    def importdat(self, widget):
        self.dataindex = 1
        self.open_file(widget)
        filename = self.root
        self.datx = loadtxt(filename, unpack = True, usecols = [0])
        self.daty = loadtxt(filename, unpack = True, usecols = [1])/100
    ...
    def plot_TR(self, widget):
        ...
        if self.dataindex == 1:
            print "Hello"
            self.ax1.plot(self.datx, self.daty, 'o')
    ...
    def findpeaks(self, x, y):
        peaks = []
        diff = [n - y[i-1] for i, n in enumerate(y)][1:]
        print diff
    ...
    def d_match(self, widget):
        peaks = self.findpeaks(self.datx, self.daty)
        #print peaks
    ...
    def about_win(self, widget):
        ...
        about.set_copyright("(c) R. E. Treharne, Sept. 2013")
        about.set_comments('(Simulated Transmission, Absorption and Reflection)')
        about.set_website("http://www.retreharne.comuv.com")
...
#Materials Window and functions -----------------------------------------------
    ...
    def __init__(self):
        ...
        datamenu = gtk.Menu()
        datam = gtk.MenuItem("Data")
        datam.set_submenu(datamenu)

        importdata = gtk.MenuItem("Import")
        #importdata.connect("clicked", self.Hello(None))
        datamenu.append(importdata)
        ...
        self.fit_win_but.connect("clicked", self.d_match)
        ...
        self.import_but = gtk.Button("Import")
        self.import_but.connect("clicked", self.importdat)
        ...
        self.box2.pack_start(self.import_but)
